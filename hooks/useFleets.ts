...
export function useFleets(fleet_id?: string) {
  ...
  const fetchFleetsUserVehicles = async () => {
    ...
    if (fleet_id) {
      ...
      // Fetch all fleets
      const { data: allFleets, error: fleetError } = await supabase.from("fleets").select("id, title, description");
    ...
    const { data: vehicleData, error: vehicleError } = await supabase
      .from("vehicles")
      .select("id, brand, sub_brand, year, plate, serial_number, economic_number, gasoline_threshold")
      .in("id", vehicleIds);
    ...
    const fleetsWithDetails = fleetData.map((fleet) => {
      const fleetRelations = relations.filter((rel) => rel.fleet_id === fleet.id);
      ...
      const vehicles = fleetRelations
        .map((rel) => vehicleData?.find((vehicle) => vehicle.id === rel.vehicle_id))
